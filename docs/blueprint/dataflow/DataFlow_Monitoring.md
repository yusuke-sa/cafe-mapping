...
| 種別 | 監視内容 | しきい値 / アクション |
| --- | --- | --- |
| APIエラー率 | HTTP 5xx, Functions失敗 | 5分平均1%以上：Slack (`#ops-alerts`) / 連続3回：メール。Azure Monitor Alert例: *Functions-Http5xx*, ResourceGroup: `rg-cafe-app`. |
| レイテンシ | `/stores`, `/favorites` のP95 | 2秒超でSlack、3秒超はメール。Alert名: *API-Latency*, メトリクス: `requests/duration`. |
| バッチ失敗 | SNS収集/AI要約/Places更新 | 1回失敗でメール、再実行後も失敗でRunbook起動。Alert名: *Batch-Failure*。 |
| Google Mapsクォータ | 80% / 100% | 80%=Slack、100%=メール+機能制限。Budget ID: `maps-monthly-budget`. |
| Instagram API | 連続失敗10回、トークン期限30日前 | Slack/メールで更新促進。Alert名: *IG-Quota*. |
