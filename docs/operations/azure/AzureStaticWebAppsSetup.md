...
| パス | 役割 | 備考 |
| --- | --- | --- |
| `src/frontend` | Next.js 15 + React 19 のフロントエンド | `appLocation` として `src/infra` から参照。 |
| `src/backend` | Azure Functions (Node.js 22) | `apiLocation` に設定可能（初期値は空文字で未連携）。 |
...
- サブスクリプション例: `cafemap-dev`。`az account set --subscription <id>` で選択。
- リソースグループ `rg-cafemap-dev`（なければ `az group create --name rg-cafemap-dev --location japaneast`）。
...
- 既定パラメータ: `location='East Asia'`, `skuName='Free'`, `appLocation='src/frontend'`, `appArtifactLocation='.next'`。  
- `repositoryToken` は `@secure()` パラメータ。渡さない場合は GitHub 連携を後で設定。

## 4. パラメータファイル
`infra/environments/dev.bicepparam` で dev 環境を定義済み。
...
必要に応じて `apiLocation` を `src/backend` に設定する。
...
### 5.2 リソースグループ確認
```bash
az group show --name rg-cafemap-dev --output table
```
存在しなければ作成する。
